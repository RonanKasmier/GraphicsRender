...
import util._
import util.{Math3D, Camera, ImageMod, Objects3D}
...
object JFrameTest extends KeyListener{
  ...
  def update(): Unit = {
    ...
    //draw axis
    try{ImageMod.drawQuickLine(canvas, cam.projectToCanvas(canvas, cam.project(Array(-20, 0, 10))), cam.projectToCanvas(canvas, cam.project(Array(20, 0, 10))), 255 << 16 | 255 << 8 | 255)}catch{case e: Exception =>}
    try{ImageMod.drawQuickLine(canvas, cam.projectToCanvas(canvas, cam.project(Array(0, -20, 10))), cam.projectToCanvas(canvas, cam.project(Array(0, 20, 10))), 255 << 16 | 255 << 8 | 255)}catch{case e: Exception =>}
    try{ImageMod.drawQuickLine(canvas, cam.projectToCanvas(canvas, cam.project(Array(0, 0, 15))), cam.projectToCanvas(canvas, cam.project(Array(0, 0, 5))), 255 << 16 | 255 << 8 | 255)}catch{case e: Exception =>}
    ...
    //val a: (Int, Int) => Boolean = (i:Int, j:Int) => true
    //points = points.sortWith((_:Array[Float], _:Array[Float]) => {true})

      //return Math3D.distanceBetweenSquared(cam.position, _) > Math3D.distanceBetweenSquared(cam.position, _)


    /*for(i <- points){
      val point: Array[Int] = cam.projectToCanvas(canvas, cam.project(i))
      ImageMod.fillRect(canvas, point(0), point(1), 255 << 16 | (255 - i%100) << 8 | 255 - i%255)
    }*/
    //things in the distance don't have to be ordered before they're rendered
    //coors[][].sortWith(((Float[], Float[]) => Bool) => {Math3D.distance(cam.position, _) < Math.3D.distance(cam.position, _)})
